  ...
  return{
    ...
    controller :function($scope, $http, $rootScope, $sce){
      ...
      $scope.sharePerscription = function(){
        $rootScope.alertsToShow(false, true, false, false);

        let prescriptionObject = $rootScope.prescriptionObjectFor1mg;
        ...
        let success = (response)=>{
          ...
          document.getElementById("hideProcessLoader").style.display= "none";
          $rootScope.alertsToShow(false, false, true, false);
        };
        let error = (error)=>{
          console.log(error);
          document.getElementById("hideProcessLoader").style.display= "none";
          $rootScope.alertsToShow(false, false, false, true);
          setTimeout(function(){
            $rootScope.alertsToShow(true, false, false, false);
            setTimeout(function(){
              document.getElementById("hideProcessLoader").style.display= "block";
            },200);
          },3000);
        };
      ...
      $rootScope.alertsToShow = function(button, loader, successmsg, failuremsg){
        $scope.showButton = button;
        $scope.showLoader = loader;
        $scope.showSuccessMsg = successmsg;
        $scope.showFailureMsg = failuremsg;
      } 

      //handling the event of terms & condition checkbox
      $scope.termsAcceptDeclineCheckbox = function(value) {
        let buyMedBtn = document.getElementById('buy_med_btn');
        if(value === false) buyMedBtn.classList.add('disable_buymed_btn');
        else if(value === true) buyMedBtn.classList.remove('disable_buymed_btn');
      }
      ...
      $scope.showTermsAndConditionUI = function() {
        ...
        $timeout(()=>{let tcScroller = new FTScroller(document.getElementById('terms_condition_mainUI'), {scrollingX: false})}, 100);
      }
