higiKioskControllers.directive('teleconsultationsummarylaborder', ['$http', 'HigiKioskFlow' ,'$timeout','HigiApiService' ,'JkioskService', 'HigiKioskStorageService', 'HigiKioskUserService', 'HigiKioskUtilitiesService'  , '$route' , function($http, HigiKioskFlow, $timeout, HigiApiService, JkioskService, HigiKioskStorageService, HigiKioskUserService, HigiKioskUtilitiesService, $route) {
  return{
    restrict: 'E',
    scope: true,
    templateUrl: 'components/modal/telemedi/ihltelemed/teleConsultation-summary-lab-order.html',
    controller :function($scope, $http, $rootScope, $sce){
      $scope.init = function(){
        $scope.showLabOrderButton = true;
        $scope.showLabOrderLoader = false;
        $scope.showOrderSuccessMsg = false;
        $scope.showOrderFailureMsg = false; 
        $rootScope.alertsToLabUIShow(true, false, false, false);
        $rootScope.showLabOrderUI = true;
        $scope.terms_condition = $rootScope.terms_condition;
        $scope.langClass = $scope.langClass || 'en_us';

        //get terms & condition html
        $http.get('docs/'+ $rootScope.termsAndPrivacyDocLang + '/' + appSettings['terms.filename'] + '.html').success(function(data){
          $scope.termsOfServicePpolHTML = data + '<a id="ppol_anchor" name="privacy"></a>';
          $http.get('docs/'+ $rootScope.termsAndPrivacyDocLang + '/' + appSettings['privacy.policy.filename'] + '.html').success(function(data){
              $scope.termsOfServicePpolHTML += data;
              $scope.termsOfServicePpolHTML = $sce.trustAsHtml($scope.termsOfServicePpolHTML);
          });
        });
      }
      ...
      $rootScope.alertsToLabUIShow = function(button, loader, successmsg, failuremsg){
        $scope.showLabOrderButton = button;
        $scope.showLabOrderLoader = loader;
        $scope.showOrderSuccessMsg = successmsg;
        $scope.showOrderFailureMsg = failuremsg;
      } 

      //handling the event of terms & condition checkbox
      $scope.termsAcceptDeclineCheckbox = function(value) {
        let lab_order_btn = document.getElementById('lab_order_btn');
        if(value === false) lab_order_btn.classList.add('disable_labOrder_btn');
        else if(value === true) lab_order_btn.classList.remove('disable_labOrder_btn');
      }

      //open terms and condition UI
      $scope.showTermsAndConditionUI = function() {
        $rootScope.keyboardHide();
        $rootScope.showLabOrderUI = false;
        $timeout(()=>{let tcScroller = new FTScroller(document.getElementById('terms_condition_mainUI_in_labOrder'), {scrollingX: false})}, 100);
      }

      //close terms and condition UI
      $scope.closeTermsAndConditionUI = function() {
        $rootScope.showLabOrderUI = true;
      }

      //share lab reports
      $scope.shareLabDetails = function() {
        $rootScope.alertsToLabUIShow(false, true, false, false);
        let prescriptionObject = $rootScope.labObjectFor1mg;
        ...
        let success = (response)=>{
          ...
          document.getElementById("hideLabOrderProcessLoader").style.display= "none";
          $rootScope.alertsToLabUIShow(false, false, true, false);
        };
        let error = (error)=>{
          console.log(error);
          document.getElementById("hideLabOrderProcessLoader").style.display= "none";
          $rootScope.alertsToLabUIShow(false, false, false, true);
          setTimeout(function(){
            $rootScope.alertsToLabUIShow(true, false, false, false);
            setTimeout(function(){
              document.getElementById("hideLabOrderProcessLoader").style.display= "block";
            },200);
          },3000);
        };
