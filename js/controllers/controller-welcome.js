higiKioskControllers.controller('HigiKioskWelcomeController', ['$interval', '$scope', '$http', '$rootScope', 'HigiKioskStorageService', 'HigiKioskFlow', 'JkioskService', 'HigiKioskUserService', '$controller', 'HigiKioskUtilitiesService', 'HigiKioskAnimationService', '$timeout', 'HigiApiService', function ($interval, $scope, $http, $rootScope, HigiKioskStorageService, HigiKioskFlow, JkioskService, HigiKioskUserService, $controller, HigiKioskUtilitiesService, HigiKioskAnimationService, $timeout, HigiApiService) {
  ...
  let socket = new WebSocket("ws://localhost:8444/paramWS/");
  ...
  $scope.mvmButtonClick = function () {
    ...
    // for invasive model manuall entry         
    //$rootScope.heamoData();
    ...
    //E-Sanjeevani session maintain 
    localStorage.setItem("flow", "E-Sanjeevani");
    let esanjeevaniData = '{"ulo":"9840698561", "upa":"tTDvYNmf"}';
    localStorage.setItem("op_lo", esanjeevaniData);
    window.location.href = "index.html";

    /* ABHA care context Enable
    $rootScope.abhaAddressFetched = "pradeep97";
  ...
  $scope.init = function () {
    ...
    if ($rootScope.selectedIvtListArray == null || $rootScope.selectedIvtListArray.length == 0) {
    ...
    $scope.invasive = "welcome.invasive";
    $scope.issueInvasive = "welcome.issueInvasive";
    $scope.ivtOfflineText = "welcome.ivtOfflineText";
    $scope.poweredbyText = "welcome.poweredbyText";
    $scope.ihlText = "welcome.ihlText";

    // Scroll for disclaimer
    $timeout(function () { $scope.scroll = new FTScroller(document.getElementById('disclaimerModal'), { scrollingX: false }); }, 2000);
          ...
          if (paymentSessionVitalTest.includes('bpw')) {
            ...
            setTimeout(() => {
              ...
              if ($rootScope.selectedVital.includes("ivt")) {
                ...
                $scope.IHLInvasiveTestButtonClass = "invasive_test_button_disabled";
              }
            }, 600);
          } else {
            ...
            setTimeout(() => {
              modulesConfigObj.forEach(element => {
                if ($rootScope.hardwareAvailability[element.hardware] == true && paymentSessionVitalTest.includes(element.mode) && $rootScope.selectedVital.includes(element.mode)) {
                  document.getElementById(element.normalClass).id = element.activeClass;
                } else {
                  if (!paymentSessionVitalTest.includes(element.mode))
                    document.getElementById(element.normalClass).id = element.deActiveClass;
                }
              });
              if ($rootScope.selectedVital.includes("ivt")) {
  ...
  /* BELOW CODES ARE DUPLICATED SO COMMENT FOR NOW IF WE HAVE FACE ANY ISSUE UNCOMMENT THIS */
  ...
  $scope.getAvaliableModuleCount = function () {
    ...
    if ($rootScope.invasiveEnable == true) {
      $scope.avaliableModuleCount++;
    }
  ...
  $scope.selectMode = function (vital) {
    ...
    /* WE HAVE ADDED THE WEIGHT AND BMC IF WEIGHT/BMC EXCLUDED */
    /*
    //Excluding the weight commented - issue code
    if ($rootScope.weightExcluded)
      $rootScope.selectedVital.push('w');
    */
    if ($rootScope.bmcExcluded)
    ...
    if ($rootScope.kioskWithPaymentMode == true && $rootScope.kioskVitalTestCost > 0 && $rootScope.kioskPaymentmodeVitalTestHomePageClickRepeat == true) {
      ...
      if (paymentSessionVitalTest.includes('bpw')) {
        ...
        if (vital.mode == 'ivt' && $rootScope.ivtServerConnection == false) {
          return 0;
        }
    ...
    if (mode == "bp") {
      ...
      if ($rootScope.invasiveEnable == true) {
        if ($scope.IHLInvasiveTestButtonClass != "invasive_test_button_active") {
          $scope.invasiveSocketConnectioCheck();
        }
      }
    }
  }
  ...
  $scope.removeFullTestActiveCalss = function (mode) {
    $rootScope.selectedVital = [];
    if (document.getElementById('bp_weight_test_btn_active') != undefined) {
      document.getElementById('bp_weight_test_btn_active').id = 'bp_weight_test_btn';
    }
    if (mode == "bp") {
      document.getElementById('bp_test_btn_active').id = 'bp_test_btn';
  ...
  $rootScope.ivtBackBtn = $scope.ivtBackBtn;

  $rootScope.ivtBackBtnWithNonIVT = function () {
    console.log($rootScope.selectedVital);
    $scope.removeFullTestActiveCalss('ivt');
  }


  $rootScope.clearActiveIvtTest = function () {
    $scope.resetIvtDashboard();
  }

  $scope.resetIvtDashboard = function () {
    if ($rootScope.kioskPaymentmodeVitalTestHomePageClickRepeat == true) {
      $rootScope.ivtIndexRemove = true;
    }
    if (document.getElementById('glucose_ivt_btn_active')) {
      document.getElementById('glucose_ivt_btn_active').id = 'glucose_ivt_btn';
    } if (document.getElementById('urine_ivt_btn_active')) {
      document.getElementById('urine_ivt_btn_active').id = 'urine_ivt_btn';
    } if (document.getElementById('hcv_ivt_btn_active')) {
      document.getElementById('hcv_ivt_btn_active').id = 'hcv_ivt_btn';
    } if (document.getElementById('pregnancy_ivt_btn_active')) {
      document.getElementById('pregnancy_ivt_btn_active').id = 'pregnancy_ivt_btn';
    } if (document.getElementById('dengue_ivt_btn_active')) {
      document.getElementById('dengue_ivt_btn_active').id = 'dengue_ivt_btn';
    } if (document.getElementById('troponin_ivt_btn_active')) {
      document.getElementById('troponin_ivt_btn_active').id = 'troponin_ivt_btn';
    } if (document.getElementById('heamoglobin_ivt_btn_active')) {
      document.getElementById('heamoglobin_ivt_btn_active').id = 'heamoglobin_ivt_btn';
    } if (document.getElementById('malaria_ivt_btn_active')) {
      document.getElementById('malaria_ivt_btn_active').id = 'malaria_ivt_btn';
    } if (document.getElementById('syphilis_ivt_btn_active')) {
      document.getElementById('syphilis_ivt_btn_active').id = 'syphilis_ivt_btn';
    } if (document.getElementById('lipid_ivt_btn_active')) {
      document.getElementById('lipid_ivt_btn_active').id = 'lipid_ivt_btn';
    } if (document.getElementById('hiv_ivt_btn_active')) {
  ...
  $scope.listPriceForVitalTest = function () {
    ...
    if ($rootScope.uniqueKioskId != undefined && String($rootScope.uniqueKioskId).trim().length > 0) {
      //let kioskID="7777";
      //alert('call');
      JkioskService.getTopScreenVideoByKioskId($scope.topScreenVideoRes, $rootScope.uniqueKioskId);
      ...
      let success_fn = function (res) {
        ...
        console.log(data);
        $rootScope.vitalPriceObjectOfThisKiosk = data;
        if (data.fullArray.length > 0) {
          ...
          if (vitalPriceObjectOfThisKiosk != undefined && vitalPriceObjectOfThisKiosk != null && Object.keys(vitalPriceObjectOfThisKiosk).length > 0) {
            if ($rootScope.vitalPayment) {
              for (let key in $scope.buttons) {
                if ($scope.buttons[key]['name'] in vitalPriceObjectOfThisKiosk) {
                  ...
                  /*if($scope.buttons[key]['name'] == "Tempeature"){
                      $scope.buttons[key]['price'] = 50;
                  } else {
                    $scope.buttons[key]['price'] = vitalPriceObjectOfThisKiosk[$scope.buttons[key]['name']];
                  }*/
                }
            ...
            if (vitalPriceObjectOfThisKiosk['machine_caption_text_list'] != undefined && vitalPriceObjectOfThisKiosk['machine_caption_text_list'] != "" && vitalPriceObjectOfThisKiosk['machine_caption_text_list'] != null) {
              var machine_caption_text_list = JSON.parse(vitalPriceObjectOfThisKiosk['machine_caption_text_list']);
              if (machine_caption_text_list['comeBackScreenText1'] != undefined)
                $rootScope.comeBackScreenText1 = machine_caption_text_list['comeBackScreenText1'];

              if (machine_caption_text_list['comeBackScreenText2'] != undefined)
                $rootScope.comeBackScreenText2 = machine_caption_text_list['comeBackScreenText2'];
            }
          ...
          //  }
          // vitalPriceObjectOfThisKiosk['other_details'] = '{    "ivt_heamo_device_batch_no": "137", "external_printer_name":"", "e_sanjeevini_enable":true, "flipkart_plus":true }';  //Old JSON format
          // if(vitalPriceObjectOfThisKiosk['other_details'] != undefined && vitalPriceObjectOfThisKiosk['other_details'] != "" && vitalPriceObjectOfThisKiosk['other_details'] != null){
          //     var other_details = JSON.parse(vitalPriceObjectOfThisKiosk['other_details']);
          //     if(other_details['ivt_heamo_device_batch_no'] != undefined)
          //         $rootScope.batchCode =  other_details['ivt_heamo_device_batch_no'];

          //     if (other_details['e_sanjeevini_enable'] != undefined)
          //       $rootScope.enableESanjeevani = other_details['e_sanjeevini_enable'];

          //       if (other_details['flipkart_plus'] != undefined) {
          //         $rootScope.flipKartPlus = other_details['flipkart_plus'];

          //         if ($rootScope.flipKartPlus)
          //           $rootScope.teleMedDisabled = true;
          //       }
          // }

          //vitalPriceObjectOfThisKiosk['other_details'] = '[{"name":"ivt_heamo_device_batch_no","value":"123","type":"number"},{"name":"e_sanjeevini_enable","value":true,"type":"boolean"},{"name":"external_printer_name","value":"","type":"text"},{"name":"flipkart_plus","value":true,"type":"boolean"}]' New JSON format

          if (vitalPriceObjectOfThisKiosk['other_details'] != undefined && vitalPriceObjectOfThisKiosk['other_details'] != "" && vitalPriceObjectOfThisKiosk['other_details'] != null) {
            var other_details = JSON.parse(vitalPriceObjectOfThisKiosk['other_details']);
            console.log(other_details);
            if (other_details.some(item => item.name == 'ivt_heamo_device_batch_no')) {
              let heamoObj = other_details.find(item => item.name === 'ivt_heamo_device_batch_no');
              $rootScope.batchCode = heamoObj.value;
            }

            if (other_details.some(item => item.name == 'external_printer_name')) {
              let externalPrintObj = other_details.find(item => item.name === 'external_printer_name');
              $rootScope.externalPrinterName = externalPrintObj.value;
            }

            if (other_details.some(item => item.name == 'e_sanjeevini_enable')) {
              let eSanjeeviniObj = other_details.find(item => item.name === 'e_sanjeevini_enable');
              $rootScope.enableESanjeevani = eSanjeeviniObj.value;
            }

            if (other_details.some(item => item.name == 'flipkart_plus')) {
              let flipKartPlusObj = other_details.find(item => item.name === 'flipkart_plus');
              $rootScope.flipKartPlus = flipKartPlusObj.value;
            ...
            if (other_details.some(item => item.name == 'abha_flow')) {
              let abhaObj = other_details.find(item => item.name === 'abha_flow');
              $rootScope.abhaflowEnable = abhaObj.value;
            }

            if (other_details.some(item => item.name == 'mehta_hospital')) {
              let mehtaObj = other_details.find(item => item.name === 'mehta_hospital');
              $rootScope.mehtaFlow = mehtaObj.value;
            }
            $rootScope.mehtaFlow = false;

            if (other_details.some(item => item.name == '')) {
              let memberObj = other_details.find(item => item.name === 'ihl_membership');
              $rootScope.ihlMembershipEnable = memberObj.value;
            }
            $rootScope.ihlMembershipEnable = true;

            if (other_details.some(item => item.name == 'mobile_input_mandatory_reg_flow')) {
              let mobileRegObj = other_details.find(item => item.name === 'mobile_input_mandatory_reg_flow');
              $rootScope.isRegFlowMobileMandatory = mobileRegObj.value;
            }

            if (other_details.some(item => item.name == 'guest_user_flow_disable')) {
              let guestUserObj = other_details.find(item => item.name === 'guest_user_flow_disable');
              $rootScope.isGuestUserFlowDisable = guestUserObj.value;
            }

            if (other_details.some(item => item.name == 'email_login_user_free_service_enable')) {
              let guestUserObj = other_details.find(item => item.name === 'email_login_user_free_service_enable');
              $rootScope.isEmailLoginUserFreeServiceEnable = guestUserObj.value;
            }
  ...
  $scope.topScreenVideoRes = function (res) {
    console.log(res);
  }
  ...
  $scope.getTotalAmountForVitalTest = function (vitals) {
    ...
    if ($rootScope.kioskInvasiveTestCost == 0) $rootScope.kioskInvasiveTestCost = 0;
    //console.log(vitals);
    ...
    if (selectedVitals.length > 0) {
      ...
      $rootScope.kioskNonInvasiveTestCost = Number(totalPriceForAllSelectedTest);
  ...
  $rootScope.ivtContinueBtn = function () {

  }

  $scope.invasiveSocketConnectioCheck = function () {
    console.log($rootScope.selectedVital);
    $rootScope.ivtDashBoardInit();
    if ($scope.IHLInvasiveTestButtonClass == "invasive_test_button_deactive") {
      ...
      socket.onopen = function (e) {
        ...
        //create a function for activating class after home click and call here instaed of direct condotion
        $scope.disableInvasiveClass();
        $rootScope.loadModal({ id: 'ivtDashboard' });
        if ($rootScope.selectedVital[0] != 'bpw')
          $scope.selectMode('ivt');
      };
      ...
      $scope.IHLInvasiveTestButtonClass = "invasive_test_button_deactive";
      // $rootScope.selectedIvtListArray = [];
      if ($rootScope.kioskPaymentmodeVitalTestHomePageClickRepeat == false) {
  ...
  /*
  $scope.disableInvasiveClass = function(){
    if($rootScope.kioskPaymentmodeVitalTestHomePageClickRepeat){
      let ivt = ["glc","urn","hcv","preg","deng","trop","heamo","mal","syph","lip","hiv"];
      $rootScope.selectedIvtListArray =  JSON.parse(localStorage.getItem("ivtListArr"));
      let invasiveTestName = "";
      ivt.forEach(function(mode) {
        if(mode == "glc"){
          invasiveTestName = "glucose";
        }else if(mode == "urn"){
          invasiveTestName = "urine";
        }else if(mode == "hcv"){
          invasiveTestName = "hcv";
        }else if(mode == "preg"){
          invasiveTestName = "pregnancy";
        }else if(mode == "deng"){
          invasiveTestName = "dengue";
        }else if(mode == "mal"){
          invasiveTestName = "malaria";
        }else if(mode == "trop"){
          invasiveTestName = "troponin";
        }else if(mode == "heamo"){
          invasiveTestName = "heamoglobin";
        }else if(mode == "syph"){
          invasiveTestName = "syphilis";
        }else if(mode == "lip"){
          invasiveTestName = "lipid";
        }else if(mode == "hiv"){
          invasiveTestName = "hiv";
        }
        var button = document.getElementById(invasiveTestName + '_ivt_btn');
        // console.log(button);
        if (button) {
          // Check if the mode is in selectedIvtListArray
          if ($rootScope.selectedIvtListArray.indexOf(mode) === -1) {
            // Disable the button
            // console.log(mode)
            button.id = invasiveTestName + '_ivt_btn'+'_disabled';
          }
        }
      });
    }
  ...
  */
  ...
  $scope.disableInvasiveClass = function () {
    if ($rootScope.kioskPaymentmodeVitalTestHomePageClickRepeat) {
      ...
      if (localStorage.getItem("ivtListArr") != true) {
        $rootScope.selectedIvtListArray = JSON.parse(localStorage.getItem("ivtListArr"));
      }
      let invasiveTestName = "";
      ivt.forEach(function (mode) {
  ...
  $scope.openMembershipModal = function(){
    ...
    $rootScope.ihlMembershipModelInit();
    $rootScope.loadModal({ id: 'memberShip' });
    return;
  }

  $scope.openABHA = function () {
    $rootScope.shareCareContextEnable = true;
