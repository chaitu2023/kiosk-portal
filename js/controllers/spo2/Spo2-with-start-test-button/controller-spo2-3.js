     ...
     //Sumithra code starts
        ...
        //Sumithra code ends
         ...
         //   $scope.nextSlide = links.next.link;
            ...
            $scope.globalSpO2CheckHealthcareProvider = "spo2three.status.spo2_check_healthcare_provider";

            $scope.oxygen = HigiKioskStorageService.returnSessionData("oxygen");
            $scope.spo2Risk = HigiKioskUtilitiesService.calculateSpO2Risk($scope.oxygen);
             $scope.spo2Status = HigiKioskStorageService.saveSessionData("spo2Status", $scope.spo2Risk);

                   if ($scope.spo2Risk == 'Healthy' ) {
            ...
            $timeout(function() {
                $scope.spo2IsHealthy    = ($scope.spo2Risk == "Healthy") ? "spo2_scale_selected" : "";
